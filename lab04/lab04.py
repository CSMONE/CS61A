...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    city = make_city('tag', lat, lon)
    d1 = distance(city1, city)
    d2 = distance(city2, city)
    if d1 < d2:
...
def replace_elem(lst, index, elem):
    ...
    return [elem * (i == index) + (i != index) * lst[i] for i in range(len(lst))]
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    """
    i = 0
    while i + num_connect <= max_cols:
        piece_set = set([get_piece(board, row, i + j) for j in range(num_connect)])
        if len(piece_set) == 1 and list(piece_set)[0] == player:
            return True
        i += 1
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    i = 0
    while i + num_connect <= max_cols:
        piece_set = set([get_piece(board, i + j, col) for j in range(num_connect)])
        if len(piece_set) == 1 and list(piece_set)[0] == player:
            return True
        i += 1
    return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    if diagonal_win \
            ...
            or check_win_row(board, max_rows, max_cols, num_connect, row, player):
        return True
    else:
        return False
