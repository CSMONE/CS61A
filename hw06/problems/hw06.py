...
def balanced(m):
    ...
    return cal_weight(branches(m)[0]) == cal_weight(branches(m)[1])
...
def make_withdraw(balance, password):
    ...
    def account(amount, password):
        nonlocal pas, bal, incorrect_list
        if len(incorrect_list) >= 3:
            return "Your account is locked. Attempts: [" + ', '.join(["'" + str(i) + "'" for i in incorrect_list]) + "]"
        if password != pas:
