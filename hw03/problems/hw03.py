...
def repeated(f, n):
    ...
    def sub_func(f1, k, m):
        if k == 0:
            ...
            return f1(sub_func(f1, k - 1, m))

    return lambda x: sub_func(f, n, x)
    """
...
def one(f):
    ...
    return lambda f1: lambda x1: f1(x1)

def two(f):
    """Church numeral 2: same as successor(successor(zero))"""
    return lambda f2: lambda x2: f2(one(f)(f2)(x2))
